...
const MessageContainer = ({ onBackUser }) => {
  ...
  useEffect(() => {
    const getMessages = async () => {
      setLoading(true);
      try {
        const tokendata = (authUser.token)
        const token = (tokenn.token)

        const get = await axios.get(`http://localhost:3000/api/v1/chats/getmessage/${selectedConversation?._id}`, {
          headers: {
  ...
  const handelSubmit = async (e) => {
    ...
    try {
      const token = (tokenn.token)
      const res = await axios.post(`http://localhost:3000/api/v1/chats/sendmessage/${selectedConversation?._id}`, { message: sendData }, {
        headers: {
          Authorization: `Bearer ${token}`, // Add the token to the headers
        },
      });
            ...
            {!loading && messages?.length > 0 && messages?.map((message) => (
              <div className='text-white' key={message?._id} ref={lastMessageRef}>
                <div className={`chat ${message.senderId === authUser._id ? 'chat-end' : 'chat-start'}`}>
                  <div className='chat-image avatar'><hr className='' /></div>
                  {/* {  console.log(message.senderId , "sender") }
      {console.log(authUser._id ,"auth") } */}
                   <div className={`chat-bubble ${message.senderId === authUser._id ? 'bg-sky-600' : ' bg-green-600'
